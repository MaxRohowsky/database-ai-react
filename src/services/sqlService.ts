...
export async function generateSql(query: string, aiConfig: AiModelConfig, dbSchema?: string): Promise<{
  ...
  try {
    ...
    const response = await window.electronAPI.generateSQL(aiConfig, query, dbSchema);
    ...
    return { 
      sqlQuery: "",
      error: error instanceof Error ? error.message : 'An error occurred generating SQL'
    };
  }
}

// Function to execute SQL query against the database
export async function executeSqlQuery(sql: string, dbConfig: ConnectionDetails): Promise<{
  ...
  try {
    ...
    if (missingFields.length > 0) {
      throw new Error(`Database configuration is incomplete. Missing fields: ${missingFields.join(', ')}`);
    }
    ...
    return { 
      error: error instanceof Error ? error.message : 'An error occurred executing SQL'
    };
  }
}

// Function to fetch database schema
export async function fetchDatabaseSchema(dbConfig: ConnectionDetails): Promise<{
  schema?: string;
  error?: string;
}> {
  try {
    ...
    return { 
      error: error instanceof Error ? error.message : 'An error occurred fetching schema'
    };
  }
} 